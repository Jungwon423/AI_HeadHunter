...
export const initialNumbersValue = (
  ...
  for (let index = 0; index < 3; index++) {
    for (let j = 0; j < numbersLength; j++) {
      ...
      if (index === 1 && j === value) {
        if ((j as number).toString().length === 1) {
          arrayOfSelectedValue.push({
            number: `0${(j as number).toString()}`,
            translatedValue: `-${count}`,
            selected: true,
          })
        } else {
          arrayOfSelectedValue.push({
            number: (j as number).toString(),
            translatedValue: `-${count}`,
            selected: true,
          })
        }
...
export const initialMinutesValue = (
  heightValue = 54,
  numbersLength = 24,
  value = 0,
) => {
  ...
  let count = 0
  for (let index = 0; index < 3; index++) {
    for (let j = 0; j < numbersLength; j += 5) {
      if ((index === 0 && j < 2) || (numbersLength === 13 && j === 0)) {
        continue
      }
      if (index === 1 && j === value) {
...
export const returnSelectedMinutes = (heightValue = 54, numbersLength = 24) => {
  const arrayOfSelectedValue: SelectedValueType[] = [
    {
      number: '00',
      translatedValue: (heightValue * 2).toString(),
